...
String outFilename = "/Users/mic/Desktop/ECG_Data.csv";
...
long lastTime = 0;  
long timerDelay = 4000;  // send readings timer

long lastTimeecg = 0;  
long timerDelayecg = 4000;  // send readings timer
...
void setup() 
{
 ...
 //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Set Screen Here>>>>>>>
  ...
  sp[0]= new Scope(0,50,50,width-100,height-100,4000,-4000,1000,100);
//  sp[0]= new Scope(0,50,10,width-100,height/2-35,1500,-1500,1000,10);
 // sp[1]= new Scope(1,50,height/2+15,width-100,height/2-35,300,-300,1000,10);
  ...
  mySoundFile = new SoundFile(this, "3.wav");
...
class Scope{
  ...
  void Plot(){
    ...
    stroke(255,0,0);
    smooth();
    strokeWeight(2); //=========================== // Plot line thickness
...
*/
  ...
  //# ECG Part
  text( "/" ,        width-46, 220 +25);
  text( BPMECG ,        width-40, 220 +25);
  
  textFont(myFont2,16);
  textSize(16); text( "Avg" , width-163, 200 +25);
  textSize(20); text( "BPM:" , width-165, 220 +25);
  textFont(myFont3,16);
  textSize(50); text( averageECG , width-110, 220 +25);
  textSize(25); text("ECG" , width -430, 32 );
  
  textFont(myFont2,16);
  textSize(17); text("Wireless Version" , width -160, 32 );
  
  textFont(myFont2,16);
//  textSize(16); text("Quanta Computer BU8" , width -790, 290 );
//  textSize(16); text("Engineer: Mic Tsai" , width -140, 290 );
  ...
  + "ECG:" + "," + valssECG + "," + "BPM:" + "," + averageECG + ",");

          if ((Alert) && (millis() - lastTime) > timerDelay )
          {  
            mySoundFile.play();
            lastTime = millis();
            Alert = false;
...
//  appendTextToFile(outFilename, "BPMECG" + BPMECG);
...
class Strage{
 ...
 /*
  void save()
  {
    String savePath = selectOutput("/Users/mic/Desktop", "123.xlsx");  // Opens file chooser
    if (savePath == null) {
...
void serialEvent(Serial myPort)
{
  ...
  //    appendTextToFile(outFilename, "ECG: " + valssECG);
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<Average ECG calculate
     ...
     /*        
          if ((millis() - lastTimeecg) > timerDelayecg )
          {  
                  if (averageECG == averageECG2)
                  {  
                      averageECG = 0;
                  } 
            lastTimeecg = millis();
